...
uid_pattern = [
    ...
    ["native", "at.stefanhuber.contactappnative"],
    ["capacitor", "at.stefanhuber.contactappcapacitor"],
    ["react", "com.contactappreactnative"],
    ["flutter", "com.thomasdorfer.contactappflutter"],
    ["gecko-wrapper", "at.stefanhuber.geckowrapper"],
    ["webview-wrapper", "at.stefanhuber.webviewwrapper"],
    ["aliexpress", "com.android.chrome"],
    ["ulangi", "com.ulangi"],
    ["yummeals", "host.exp.exponent"],
    ["bustimetable", "com.davituri.timetable"],
    ["newpipe", "org.schabi.newpipe"],
    ["fdroid", "org.fdroid.fdroid"],
    ["tinyweather", "de.kaffeemitkoffein.tinyweatherforecastgermany"],
    ["flutterebook", "dev.jideguru.flutterEbookApp"],
    ["spacexgo", "com.chechu.cherry"],
    ["inkino", "com.roughike.inkino"],
    ["ytmusic", "com.android.chrome"],
    ["letsplant", "com.android.chrome"],
    ["devto", "com.android.chrome"],
    ["letgo", "com.android.chrome"],
    ["swiggy", "com.android.chrome"],
    ["versus", "com.android.chrome"],
    ["swiggyfirefox", "org.mozilla.firefox"],
    ["versusfirefox", "org.mozilla.firefox"],
    ["aliexpressfirefox", "org.mozilla.firefox"],
    ["letgofirefox", "org.mozilla.firefox"],
    ["sworkit", "sworkitapp.sworkit.com"],
    ["metalreleases", "org.onepointzero.metalreleases"],
    ["allman", "com.kurtlourens.no_mans_sky_recipes"],
    ["untapped", "com.untappdllc.app"],
    ["amtrak", "com.amtrak.rider"],
    ["justwatch", "com.justwatch.justwatch"]
]

def findUid(app, result):
    for pattern in uid_pattern:
        if app == pattern[0] and result["top_pkg"] == pattern[1]:
    ...
    if uid:
        ...
        if value > 0.001:
            ...
            if app not in data[device].keys():
                data[device][app] = {}
            if interaction not in data[device][app].keys():
                data[device][app][interaction] = {"test": [], "app": [], "test_joule": [], "app_joule": [], "voltage": []}

            data[device][app][interaction]["voltage"].append(float(result["volt"]) / 1000)
...
def reformat_data(result):
    ...
    app_names = {
        ...
        "capacitor": "Capacitor",
        "webview-wrapper": "PWA (Chrome Wrapper)",
        "firefox-wrapper": "PWA (Firefox Wrapper)",
    }
    ...
    with open(f, "rt") as file:
        ...
        for line in file:
            if state == "i" and re.match("^$", line):
                ...
                m = re.match('^ +[0-9+msh]+ \([0-9]+\) [0-9]{3,} top=([a-z0-9]+):"([a-zA-Z0-9._]+)"', line)
                if m:
